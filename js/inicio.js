...
function llenarMiContra() {
  // console.log("Se debe de llenar las etiquetas");
  $.ajax({
    url: "../inicio/miContra.php",
    data: {},
    type: "POST",
    dataType: "html",
    success: function(respuesta) {
        ocultarSec();
      $("#miContra").html(respuesta);
      $("#miContra").hide();
      $("#miContra").fadeIn("slow");
    },
    error: function(xhr, status) {
      alert("no se muestra");
    }
...
function QuitarClass(){
  ...
  $("#linkMiContra").removeClass("activo");
}
...
$("#linkMiContra").on("click", function() {
  llenarMiContra();
  QuitarClass();
  $("#linkMiContra").addClass("activo");
});